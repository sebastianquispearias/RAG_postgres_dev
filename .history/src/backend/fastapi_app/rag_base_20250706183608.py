...
class RAGChatBase:
    ...
    def get_chat_params(self, messages: list[Message], overrides: ChatRequestOverrides) -> ChatParams:
            """
            Procesa los mensajes de la petición y las configuraciones para crear un nuevo objeto ChatParams.
            Esta versión corregida evita el error de valores múltiples.
            """
            # Crea un diccionario a partir de las configuraciones
            overrides_dict = overrides.model_dump()
            
            # Obtiene el prompt_template desde el diccionario, o usa el por defecto.
            # Crucial: lo elimina del diccionario para no pasarlo dos veces.
            ...
            return ChatParams(
                **overrides_dict, # Ahora este diccionario ya no tiene 'prompt_template'
                original_user_query=messages[-1].content,
                past_messages=past_messages,
                prompt_template=prompt_template, # Se pasa aquí, una sola vez
    ...
    def prepare_rag_request(self, query: str, results: list[AbastecimentoPublic]) -> str:
            sources = ""
            # La numeración de los documentos ahora empieza en 1 para que coincida con las citas
